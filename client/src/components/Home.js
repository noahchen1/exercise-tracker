...
export default function Home() {
  ...
  useEffect(() => {
    ...
    for (var i = 0; i < 7; i++) {
      dateArray.push(
        new Date(curr.setDate(firstDate.getDate() + i)).toLocaleDateString()
      );
    }
    ...
    if (weight[username] <= 125) {
      const totalCaloriesToday = durationsToday
        .map((v, i) => (v / 30) * data125lbs[activitiesToday[i]])
        .reduce((x, y) => parseInt(x + y), 0);
      setCaloriesToday(totalCaloriesToday);
    } else if (weight[username] > 125 && weight[username] <= 155) {
      const totalCaloriesToday = durationsToday
        .map((v, i) => (v / 30) * data155lbs[activitiesToday[i]])
        .reduce((x, y) => parseInt(x + y), 0);
      setCaloriesToday(totalCaloriesToday);
    } else {
      const totalCaloriesToday = durationsToday
        .map((v, i) => (v / 30) * data185lbs[activitiesToday[i]])
        .reduce((x, y) => parseInt(x + y), 0);
      setCaloriesToday(totalCaloriesToday);
    }
    ...
    const totalCal = dailyActivity.map((duration, i) => {
      ...
      return duration
        .map((v, j) => (v / 30) * eachDayCalories[j])
        .reduce((x, y) => parseInt(x + y), 0);
    });
    ...
    const getUserAge = () => {
      const differenceFromToday =
        new Date() - new Date(birthday[username]).getTime();
    ...
    const getUserBMR = () => {
      ...
      if (userGender[0].toLowerCase() == "male") {
        const BMR =
          65 +
          6.23 * weight[username] +
          12.7 * height[username] * 0.39 -
          6.8 * userAge;
        return BMR;
      } else if (userGender[0].toLowerCase() == "female") {
        const BMR =
          655 +
          4.35 * weight[username] +
          4.7 * height[username] * 0.39 -
          4.7 * userAge;
        return BMR;
      }
    };
    ...
    const energyExpandedBarLength = totalCalAllWeek.map(item =>
      (item / highestEnergyExpand).toFixed(2)
    );
          ...
          {isDesktop && (
            ...
            >
              <div>
                ...
                <span
                  style={{ color: "rgba(0, 0, 0, 0.7)", marginLeft: "5px" }}
                >
                  Cal
                </span>
                <div
          ...
          {isSmallScreen && (
            ...
            >
              <div>
                ...
                <span
                  style={{ color: "rgba(0, 0, 0, 0.7)", marginLeft: "10px" }}
                >
                  Cal
                </span>
                <div
